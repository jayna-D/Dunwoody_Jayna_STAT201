...
num_count <- 0
while (num_sum < 30) {
  num <- sample(1:10, 1, replace=TRUE)
  print(num)
  num_sum <<- num_sum + num
  num_count <<- num_count + 1
}
print(sprintf("You generated %d numbers that sum to %d.", num_count, num_sum))
```
...
BMI <- bmi_calculator(75,165)
print(sprintf("Your BMI is %s with a value of %s", BMI[2], BMI[1]))
```
...
```{r}
mat <- matrix(c(vec1, vec2, vec3), 5, 3)
print(mat)
```
...
```{r}
list_object <- list(vec1, vec2, vec3)
print(list_object)
```
...
```{r}
even_odd <- (sapply(list_object, function(x) sum(x))) %% 2 == 0
print(even_odd)
```
...
```{r}
for (row in 1:nrow(fruit_info)) {
  if (fruit_info[row,"weight"] > 100) {
    print(paste("The", fruit_info[row, "fruit"],"is heavy"))
  } else {
    print(paste("The", fruit_info[row, "fruit"],"is light"))
  }
}
```
