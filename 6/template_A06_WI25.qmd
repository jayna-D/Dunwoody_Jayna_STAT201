...
spread_index = which.max(win_spread)
sprintf("The largest win spread is %d, when the Bears scored %d points and the Opponent scored %d points.", win_spread[spread_index], bears[spread_index], opponent[spread_index])
```
...
```{r}
missing_score <- matrix(c(37, 9, 17, 17), ncol = 2)
game_score <- rbind(game_score, missing_score)
print(game_score)
```
...
```{r}
for (score in 1: nrow(game_score)){
  if (game_score[score,1] > game_score[score,2]) {
    print("Win")
  }  else {
    print("Loss")
  }
...
```{r}
for (row in 1: nrow(game_score)){
  if (game_score[row,3] == 1) {
    cat("The Bears lost",row - 1,"games before their first win.")
    break
  }
}
```
...
```{r}
area_of_rect <- function(len=48, wid=48) {
  area_in <- len * wid
  area_ft <- (len * wid)/12
...
area <- area_of_rect()
cat(sprintf("The area in square inches is %d.", area[[1]]), sprintf("The area in square feet is %d.", area[[2]]), sep="\n")
```
...
in_vector <- is_in_vector(value, check)
if (in_vector) {
...
convert_even_odd <- function(matrix) {
  even_index <- (matrix %% 2 == 0)
  odd_index <- (matrix %% 2 != 0)
  matrix[even_index] <- 1
  matrix[odd_index] <- 0
  return(matrix)
}
